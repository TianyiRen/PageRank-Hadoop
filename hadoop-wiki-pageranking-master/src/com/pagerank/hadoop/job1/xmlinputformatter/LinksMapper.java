...
public class LinksMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
        ...
        Matcher categoryMatcher = categoryPattern.matcher(titleAndText[1]);
        ...
        if(entered){
            ...
            while (categoryMatcher.find()) {
            	String cat = categoryMatcher.group(0).replace("[[Category:", "");
            	cat = cat.replace("]] ", ";");
            	cat = cat.replace("]]", ";");            	
            	category = category + cat;            	
            }        	
        	output.collect(page, new Text( "#" + category + "#"));
        }
